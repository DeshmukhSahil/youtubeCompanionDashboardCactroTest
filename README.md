...
* OAuth refersh-token script: `oauth-get-tokens.js` -script to obtain refresh token
...
https://www.loom.com/share/2a90a8aae8344721b6a65a2606dc4444
...
OAUTH_REDIRECT_URI=http://localhost:7000/oauth2callback
# OAuth tokens (write permissions)
YOUTUBE_ACCESS_TOKEN=<INITIAL_ACCESS_TOKEN>
YOUTUBE_REFRESH_TOKEN=<REFRESH_TOKEN_FROM_GOOGLE>

# Optional (auto-managed — will be overwritten at runtime)
YT_ACCESS_EXPIRY=0
...
5. (Optional) Run the OAuth helper to get tokens:
   ...
   * Open `http://localhost:7000/auth`, complete consent, and copy the returned tokens into your `.env`.
6. `node server.js` or `npm start`
7. Visit `http://localhost:7000/` to open the UI (or call API endpoints with Postman/curl).
...
```bash
curl http://localhost:7000/api/video
```

Post a comment:

```bash
curl -X POST http://localhost:7000/api/comment \
...
```bash
curl "http://localhost:7000/api/comments?maxResults=50"
```

Delete comment:

```bash
curl -X DELETE http://localhost:7000/api/comment/COMMENT_ID
```

Create note:

```bash
curl -X POST http://localhost:7000/api/note \
  -H "Content-Type: application/json" \
